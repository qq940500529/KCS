...
生成私钥 → 生成多个转换密钥（1-5个） → 获取 TPM 核心密钥 → 
...
2. 验证每个转换密钥的哈希值是否正确
3. 只有所有转换密钥都正确时才能重建主密钥
4. 任何一个转换密钥错误或缺失都会导致解密失败
...
解密需要同时满足：
1. 正确的公钥
2. **所有转换密钥都正确**（支持 1-5 个，必须全部提供且正确）
3. 正确的服务器（硬件 + URL）
4. 正确的时间窗口
...
- ✅ 实现多人授权机制（每人持有一个转换密钥）
- ✅ 提高安全性（攻击者需要获取所有转换密钥）
- ✅ 防止单点失败（单个密钥持有者无法独自解密）
- ✅ 支持组织授权策略（如 2/3 人员同时授权）
