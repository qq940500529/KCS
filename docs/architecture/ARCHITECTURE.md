...
生成私钥（包含时间窗口） → 生成多个转换密钥（数量可配置） → 获取 TPM 核心密钥 → 
为每个转换密钥派生子密钥 → 组合所有子密钥生成主密钥 (KDF) → 
加密私钥（时间窗口也被加密） → 生成公钥（包含转换密钥数量和时间窗口信息） → 返回结果
```
...
**私钥结构增强**：
- 私钥不再是简单字符串，而是包含密钥字符串和时间窗口的 JSON 结构
- 时间窗口信息被加密保存在私钥中，与公钥中的时间窗口形成双重验证
- 防止攻击者通过修改公钥时间窗口来绕过时间限制
...
6. **时间窗口一致性校验**：解密后验证私钥中的时间窗口与公钥中的时间窗口是否一致
...
### 4.4 防止攻击
- **离线攻击**：转换密钥不存储，需要所有密钥才能解密，无法暴力破解
- **部分密钥泄露**：即使泄露部分转换密钥，仍需要所有密钥才能解密
- **硬件克隆**：TPM 绑定防止硬件迁移
