...
| **转换密钥 (Transfer Key)** | 系统随机生成 (**可配置数量，至少 1 个**)                  | **不存储**，需分享给接收者       | 公钥 -> 私钥转换的授权凭证       | **解密时必须提供所有转换密钥**，顺序无关，缺一不可 |
...
2.  **生成转换密钥**：
    - 随机生成 **任意数量的转换密钥** $K_{trans,1}, K_{trans,2}, ..., K_{trans,n}$（数量可配置，至少 1 个）。
    - 每个转换密钥独立生成，具有相同的安全强度（256 位）。
    - 建议根据实际场景配置：2-3 个适合小团队，5-10 个适合中型组织。
3.  **定义时间窗口**：
