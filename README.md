...
#### ⏰ 防篡改时间控制
- **TPM 时钟验证**：依赖 TPM 内部防篡改时钟，不受系统时间影响
- **应用层时间绑定**：时间窗口参与密钥派生过程，篡改公钥中的时间会导致解密失败
- **精确时间窗口**：支持设置生效和截止时间，超出窗口自动拒绝解密
- **TPM Policy 强制**：可选配置 TPM Policy（PolicyCounterTimer）在硬件层强制时间窗口，攻击者即使修改应用代码也无法绕过（需部署配置）
...
| **私钥<br>(Private Key)** | 加密级随机数生成器<br>(6-16 位) | 用户自行保管 | 文件加密 | • 包含大小写字母、数字、符号<br>• 仅生成时显示一次 |
| **转换密钥<br>(Transfer Keys)** | 256 位随机数生成<br>（数量可配置，至少 1 个） | **不存储**，<br>用户分发给授权人 | 公钥→私钥转换的<br>多重授权凭证 | • 全部必需，缺一不可<br>• 顺序无关<br>• 支持任意数量 |
| **公钥<br>(Public Key)** | 系统生成<br>(Base64 编码的 JSON) | 用户分享 | 携带加密的私钥、<br>时间窗口、验证信息 | • 包含转换密钥哈希<br>• URL 绑定<br>• 时间绑定 |
...
生成密钥后，前端提供便捷的导出和分发功能：

1. 公钥：可直接复制或导出为 `.pub` 文件
2. 转换密钥：每个密钥可单独复制或导出为 `.tkn` 文件，支持批量导出为 ZIP
3. 私钥：仅生成时显示一次，可复制但不提供文件导出
...
| **时间层** | TPM 防篡改时钟 + 应用层绑定 + TPM Policy（可选） | 防止时间篡改，应用层保护+硬件层强制 |
...
| **公钥时间篡改** | 修改公钥中的时间窗口延长访问期限 | 时间窗口通过核心密钥嵌入派生过程，篡改导致解密失败 | ❌ 失败 |
| **代码修改（应用层）** | 修改验证逻辑跳过时间检查 | 时间参与密钥派生，应用层修改无法解密 | ⚠️ 应用层受限 |
| **代码修改（TPM Policy）** | 修改代码绕过时间检查并返回私钥 | 配置 TPM PolicyCounterTimer，密钥不经 policy 验证无法使用 | ❌ 失败（需配置） |
| **部分泄露** | 获取部分转换密钥 | 需要全部转换密钥，缺一不可 | ❌ 失败 |
| **中间人** | 拦截网络通信 | 强制 HTTPS，证书验证 | ❌ 失败 |
| **内部威胁** | 管理员滥用权限 | 审计日志，最小权限原则，TPM Policy 限制 | ⚠️ 受限 |

### TPM Policy 强制时间窗口（高级安全配置）

当需要对抗服务器完全控制权限下的代码修改攻击时，可配置 TPM Policy（PolicyCounterTimer）实现硬件层强制时间窗口控制：

**安全边界说明**：
- **应用层时间控制**（当前默认）：时间窗口参与密钥派生，修改公钥时间会导致解密失败，但攻击者若修改应用代码可绕过时间检查直接返回私钥
- **TPM Policy 强制**（可选配置）：将解密权限下沉到 TPM 硬件，使用 PolicyCounterTimer 约束，攻击者即使修改应用代码也无法在时间窗外使用密钥

**实现路线（方案一）**：
1. 创建 sealed object 存储解密所需的关键材料/种子
2. 为 sealed object 设置 authPolicy，包含 PolicyCounterTimer 约束
3. 运行时必须满足 policy session 才能 unseal，获取密钥材料
4. 时间窗外 TPM 直接拒绝 unseal，应用层无法绕过

**前提条件**：
- 真实 TPM 2.0 硬件（swtpm 仅用于开发测试）
- 对象 authPolicy 正确绑定
- 密钥材料不以明文离开 TPM
- 禁用 debug/导出路径
- 适当的部署隔离
...
**限制与注意事项**：
- PolicyCounterTimer 使用 TPM clock/counter（相对时间），非 UTC 绝对时间
- 需要映射 UTC 时间窗到 TPM tick 范围
- TPM 重启/复位会影响计数器
- 表达能力有限，复杂时间策略可能需要 PolicySigned 或多 policy 组合

详细实现步骤和代码示例请参考：
- [TPM 集成指南 - PolicyCounterTimer 详细实现](docs/guides/TPM_INTEGRATION.md#6-tpm-policy-时间窗口强制)
- [安全性设计 - TPM Policy 强制时间窗](docs/architecture/SECURITY.md#5-tpm-policy-强制时间窗口)
