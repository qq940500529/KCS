...
- **TPM Policy 硬件强制**：使用 TPM Policy（PolicyCounterTimer）在硬件层强制时间窗口，攻击者即使修改应用代码也无法绕过
...
| **时间层** | TPM 防篡改时钟 + TPM Policy 硬件强制 + 应用层绑定 | 防止时间篡改，硬件层强制+应用层保护 |
...
| **代码修改（TPM Policy）** | 修改代码绕过时间检查并返回私钥 | TPM PolicyCounterTimer 硬件强制，密钥不经 policy 验证无法使用 | ❌ 失败 |
...
系统采用 TPM Policy（PolicyCounterTimer）实现硬件层强制时间窗口控制，对抗服务器完全控制权限下的代码修改攻击：

**安全边界说明**：
- **应用层时间控制**：时间窗口参与密钥派生，修改公钥时间会导致解密失败，但攻击者若修改应用代码可绕过时间检查直接返回私钥
- **TPM Policy 强制**：将解密权限下沉到 TPM 硬件，使用 PolicyCounterTimer 约束，攻击者即使修改应用代码也无法在时间窗外使用密钥
